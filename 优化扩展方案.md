...
> 日期：2026-07-31 · 状态：**P0–P4 完成**；**P5 可信度/可发现性 → 0.3.3**  
> 前置：v0.3.2 已发（稳定向 + 动画体验）。  
> 真相源仍以 `进度记录.md` / 代码为准；本文是下一阶段排期与设计。

## 0. 范围

**已做（P0–P4）：** 工程卫生、应用内 hooks、默认帧动画、缩放/重置、终态提醒、会话轻列表、适配器校准、文案/skins.json、发版资产校验、hooks 路径失效、预览图排除。

**P5 做（可信度 + 可发现性）：** 久无事件提示、启动 hooks 需补装轻提醒、托盘菜单指纹/防抖、渲染按 fps 跳帧与不可见暂停。

**明确不做（与 AGENTS.md 一致）：** 多宠、Live2D/Spine、跨平台、像素级点击穿透。

```mermaid
flowchart TB
  done["P0–P4 / 0.3.2"] --> p5["P5 可信度 → 0.3.3"]
  p5 --> later["P6 新皮肤 / 内容（0.4）"]
```

跨阶段硬约束：Push 模型；`pet-bridge` 永远 `exit(0)` + ~1s；单端口 4271；Agent 差异只进 `adapters/`；新 Settings 字段一律 `serde(default)`。
...
- 条件：`connectionHints` 开 + idle +（已引导或已装 hooks）+ 连续 `silenceHintMs`（默认 30min）无事件
- 气泡：「好像没接到 Agent · 点我打开连接设置」；点一次打开配置并本轮不再提示（有新事件后重置）
- Settings：`connectionHints`（默认 true）、`silenceHintMs`

### P5.2 启动自检

- 启动后 `hooks_status`：任一源 `needsUpdate` → 气泡「连接需补装」；优先于静默提示
- 与 P5.1 共用 `connectionHints` 开关

### P5.3 托盘菜单节流

- `menu_fingerprint`：会话键/标签 + 过滤 + 皮肤 + 自启；未变跳过重建
- 有变则 400ms 合并重建（`refresh_menu`）

### P5.4 渲染节流

- `PetRenderer`：按素材 fps（上限 30）或变换动画 12fps 跳帧
- `visibilitychange` 隐藏时停 rAF，可见时恢复

**验收：** 久无事件出现一次提示可关；需补装启动可见；高频同会话事件不狂刷托盘；`tsc` / 单测 / Rust 全绿。
...
- `scripts/check-release-assets.mjs`：断言 draft/正式 release 含 `.exe` / `.sig` / `latest.json`
- `release.yml` 在 tauri-action 后跑校验
- README：draft 窗口说明 + 安装版冒烟清单

### P4.2 Hooks 健康度

- `hooks_status`：配置里 `pet-bridge.mjs` 路径不存在 → `needsUpdate` + 补装提示（优先于「缺事件」）
- 配置面板：无 Node / 无 adapters 黄字提示（已有缺事件黄灯）

### P4.3 体积与卫生

- `skins.ts` glob 排除 `_preview*.png`，预览图不进前端包

### P4.4 人工冒烟（不自动化）

见 README「安装版冒烟」；发 0.3.2 正式版前至少跑一遍。

**验收：** CI 缺资产会失败；路径失效可在设置里看到并一键补装；预览图不进 dist。

---

## 已完成摘要（P0–P3）

详见下文历史章节与 `进度记录.md`。排期表中 P3 对应版本已调整为合入 0.3.0，体验补丁为 0.3.1。
