...
> 日期：2026-07-30 · 状态：**P0–P3 完成**；**P4 稳定向进行中**（0.3.1 已发；下一补丁 0.3.2）  
> 前置：v0.2.0 帧动画已发布；智能文案旁路（规则/OpenAI，默认关）已落地。  
> 真相源仍以 `进度记录.md` / 代码为准；本文是下一阶段排期与设计。

## 0. 范围

**已做（P0–P3）：** 工程卫生、应用内 hooks、默认帧动画、缩放/重置、终态提醒、会话轻列表、适配器校准、文案/skins.json。

**P4 做（稳定优先，暂不做新皮肤）：** 发版资产校验、hooks 路径失效检测、预览图不进包、安装版冒烟清单、配置面板缺 Node/adapters 提示。

**明确不做（与 AGENTS.md 一致）：** 多宠、Live2D/Spine、跨平台、像素级点击穿透。

```mermaid
flowchart TB
  done["P0–P3 已完成 / 0.3.1"] --> p4["P4 稳定向 → 0.3.2"]
  p4 --> later["择机：新皮肤 / 内容（0.4）"]
```
