...
impl EventRouter {
    /// 记录会话活跃并进行状态去重；状态有变化时返回 Some(ev) 以供转发
    pub fn route(&self, ev: AgentEvent) -> Option<AgentEvent> {
        ...
        if last.get(&key).map(String::as_str) == Some(ev.state.as_str()) {
            return None; // 状态未变化，去重
        }
        last.insert(key, ev.state.clone());
        Some(ev)
    }

    /// 列出当前活跃会话（剔除过期），最近活跃在前
